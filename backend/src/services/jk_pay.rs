...
async fn do_jk_pay(
    ...
    for attempt in 0..2u32 {
        ...
        .await
        {
            ...
            Err(e) => {
                let _: () = redis.del(TOKEN_REDIS_KEY).await.unwrap_or(());
                tracing::warn!("[JK Token] cleared cached token due to pay error: {}", e);
                Err(e)
            }
        };
    }
...
async fn try_pay_with_password(
    ...
    // Step 2: Verify health card and query payment channels
    ...
    // Step 3: Pre-calculate
    let enc_pwd2 = make_card_password(card_no, card_password)?;
    ...
    let status_req = json!({"xrefNo": order_no, "type": 20});
    let status_req_str =
        serde_json::to_string(&status_req).map_err(|e| format!("json error: {e}"))?;

    let status_data = jk
        .api(
            "baize.drugCardQueryOrderStatus",
            &[("req", &status_req_str), ("sellerId", SELLER_ID)],
            wtk,
        )
        .await?;
    ...
    let order_status = s.get("status").and_then(|v| v.as_i64());
...
mod tests {
    ...
    async fn test_jk_pay_001() {
        ...
        let redis_url = std::env::var("REDIS_URL")
            .unwrap_or_else(|_| "redis://default:liu5tgb%5ETFC@127.0.0.1:6379".to_string());
