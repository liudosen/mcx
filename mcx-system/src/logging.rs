...
impl LoggingConfig {
    pub fn from_env() -> Self {
        ...
        let app_keep_days = std::env::var("LOG_APP_KEEP_DAYS")
            .unwrap_or_else(|_| "3".to_string())
            .parse::<u32>()
            .unwrap_or(3);

        let error_keep_days = std::env::var("LOG_ERROR_KEEP_DAYS")
...
pub fn init_logging(config: Option<LoggingConfig>) {
    ...
    std::fs::create_dir_all(&app_daily_log_dir).expect("Failed to create app log directory");
    std::fs::create_dir_all(&error_daily_log_dir).expect("Failed to create error log directory");
